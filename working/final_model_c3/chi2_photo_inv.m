...
try
    sim_m=model_fun_c3_inv(v,free_pars);
...
%     chi2 = sum(a1+a2+a3);
% chi2 = sum((wAn.*((an_obs-an_sim)./max(abs(an_obs),eps)).^2)+...
%         (wNPQ.*(npq_obs-npq_sim)./max(abs(npq),eps).^2)+...
%         (wFs.*(fs_obs-fs_sim)./max(abs(fs_obs),eps).^2));
catch
