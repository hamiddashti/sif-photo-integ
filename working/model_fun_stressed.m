function m = model_fun_stressed(ci,vcmax,v)
...
eta = (1-(nl./nc)+(3+7.*gammas./ci)./((4+8.*gammas./ci).*nc)); % PS I/II ETR
...
JP680_j = JP700_j./eta;
Vc_j = JP680_j./(4.*(1+2.*gammas./ci));
Vo_j = Vc_j.*2.*gammas./ci;
Ag_j = Vc_j - Vo_j./2;

% Expressions for potential Rubisco-limited rates (_c)
%   N.B., see Eqns. 32-33
Vc_c = ci.*Vcmax./(ci + Kc.*(1+O./Ko));
Vo_c = Vc_c.*2.*gammas./ci;
Ag_c = Vc_c - Vo_c./2;
JP680_c = Ag_c.*4.*(1+2.*gammas./ci)./(1-gammas./ci); 
JP700_c = JP680_c.*eta;
...
% Select minimum Ag_a 
Ag_a  = min(tr(Ag_j,Ag_c,theta1),[],2).*logical(ci>gammas) + ...
               max(tr(Ag_j,Ag_c,theta1),[],2).*logical(ci<=gammas);
An_a = Ag_a - Rd;
