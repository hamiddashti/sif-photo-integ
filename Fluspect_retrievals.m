function [er,params0,params1,spectral,reflFluspect,tranFluspect,leafopt,leafbio,leafopt_all, leafbio_all]= Fluspect_retrievals(leafbio,c,include,target,measured,wl,wlrange,optipar)
...
if length(c)>1
    [leafbio_all.Cab,leafbio_all.Cw,leafbio_all.Cdm, leafbio_all.Cs,leafbio_all.Cca,leafbio_all.N,leafbio_all.Cx,leafbio_all.Cant,leafbio_all.fqe_opt,leafopt_all.er2] = deal(zeros(length(c),1));
    [leafopt_all.refl,leafopt_all.tran,leafopt_all.kChlrel] = deal(zeros(length(spectral.wlP),length(c)));
...
params0(8)      = leafbio.Cant;

LB              = [0   .001   0    0   0 1   -1 0]'; % lower boundaries
UB              = [100 0.5  0.4 0.6   30 4  3  10]'; % upper boundaries
...
for j = c
    ...
    if length(c)>1
        ...
        leafopt_all.kChlrel(:,j) = leafopt.kChlrel;
