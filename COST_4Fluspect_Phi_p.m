function [ertot,leafopt2] = COST_4Fluspect_Phi_p(params,measurement,input,P)
leafbio_all = input{1};
optipar     = input{2};
spectral    = input{3};
target      = input{4};

%L = length(leafbio_all.Cab);
%leafbio_all.Cab = params(1:L);
%optipar.phi = interp1(P.CoeffsKcaLength,params(L+1:end)',spectral.wlP)';%params';
...
for k = 1:length(leafbio_all.Cab)
    ...
    switch target
        case 0
            ...
            %fqeopt = (measurement.Fu(jj,k) + measurement.Fd(jj,k))\ (leafopt2.Fu(:,k) + leafopt2.Fd(:,k));
